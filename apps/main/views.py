...
class MyPostsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return models.Post.objects.filter(author=self.request.user).select_related('author', 'category')
...
def post_by_category(request, slug):
    ...
    posts = models.Post.objects.filter(category=category, status='published').select_related('author', 'category').order_by('-created_at')
...
def popular_posts(request):
    posts = models.Post.objects.filter(status='published').select_related('author','category').order_by('-views_count')[:10]
    serializer = serializers.PostSerializer(posts, many=True, context={'request': request})
    return Response(serializer.data)
...
def recent_posts(request):
    posts = models.Post.objects.filter(status='published').select_related('author','category').order_by('-created_at')[:10]
    serializer = serializers.PostSerializer(posts, many=True, context={'request': request})
    return Response(serializer.data)